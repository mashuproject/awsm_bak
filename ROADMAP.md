...
Library and Search Materializations now follow the same canonical local boundary: their
Frontier-bound snapshots are installation-wrapped before entering the Go state store, and a
missing, stale, or unauthentic snapshot rebuilds from the authenticated Replica. Remaining
evidence work is deliberately narrow: extend the shared cross-language corpus beyond its current
Recovery and Key Epoch derivation set to the remaining canonical command/replay and Complete
Export boundaries, and keep the randomized multi-Replica, restart/crash, authority/conflict,
Sparse hydration, and native-package proofs aligned as the semantic boundary grows.
