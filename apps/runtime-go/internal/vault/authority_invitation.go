...
// cancelInvitation records an externally verified type-7 Cancellation Event.
// The Redemption Authority remains responsible for producing the receipt; the
// Runtime only authenticates that receipt as part of Vault replay.
func (r *Runtime) cancelInvitation(ctx context.Context, id, encodedRequest, encodedReceipt string) (any, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	before := r.snapshotLocked()
	if err := r.requireExpectedLocked(&id); err != nil {
	...
	if value.Lifecycle != "Open" {
		return nil, commandError("VAULT_READ_ONLY", "A closed Vault cannot record Invitation cancellation.")
	}
	if value.Canonical == nil || !value.Canonical.AuthoringAvailable || r.replicas[id] == nil || r.deps.Artifacts == nil || r.deps.Secrets == nil {
		return nil, commandError("VAULT_REPLAY_UNAVAILABLE", "The authenticated authoring Client Credential is unavailable.")
	}
	decodeValue := func(encoded, label string) (canonical.Value, error) {
		decoded, decodeErr := base64.RawURLEncoding.DecodeString(encoded)
		if decodeErr != nil {
	...
	request, err := decodeValue(encodedRequest, "The Cancellation Request")
	if err != nil {
		return nil, err
	}
	receipt, err := decodeValue(encodedReceipt, "The Cancellation receipt")
	if err != nil {
		return nil, err
	}
	vaultID, err := decodeHexIdentifier(id)
	...
	event, err := canonical.SignEvent(canonical.EventInput{
		...
		AuthorityParentIDs: replicaState.AuthorityFrontier, Dependencies: nil,
		RequiredFeatureSetID: featureSetID, Extensions: map[string][]byte{}, Family: canonical.AuthorityFamily,
		Type: 7, SignerCredentialID: credentialID, AssertedAt: time.Now().UnixMilli(),
		Body: canonical.Map{0: request, 1: receipt},
	}, ed25519.PrivateKey(clientSecret.signingSecretKey))
	if err != nil {
		return nil, commandError("VAULT_EVENT_INVALID", "The Invitation Cancellation Event could not be authored.")
	}
	parsed, err := parseInvitationCancellation(event)
	if err != nil {
		return nil, commandError("INVITATION_INVALID", "The Invitation Cancellation payload is invalid.")
	}
	encoded, err := awsmcrypto.SealCompactItem(awsmcrypto.CompactItemInput{VaultID: vaultID, KeyEpochID: epochID, KeyEpochKey: epochSecret.key, PayloadType: 1, PayloadBytes: event.Bytes})
	if err != nil {
		return nil, commandError("VAULT_EVENT_INVALID", "The Invitation Cancellation Event could not be protected.")
	}
	envelope, err := storage.DecodeOpaqueEnvelope(encoded)
	if err != nil {
		return nil, commandError("VAULT_EVENT_INVALID", "The Invitation Cancellation envelope is invalid.")
	}
	if err := storeOpaqueCreationItem(r.deps.Artifacts, envelope.StorageItemID, encoded); err != nil {
		return nil, commandError("VAULT_CREATION_STORAGE_FAILED", "The Invitation Cancellation Event could not be stored.")
	}
	cleanup := func() { deleteOpaqueCreationItem(r.deps.Artifacts, envelope.StorageItemID) }
	nextReplica := r.replicas[id].Clone()
	if err := nextReplica.AdmitEvent(event, ed25519.PublicKey(clientSecret.signingPublicKey)); err != nil {
		cleanup()
		return nil, commandError("VAULT_EVENT_INVALID", "The Invitation Cancellation receipt could not be authenticated.")
	}
	if value.Canonical.RecordStorageItemIDs == nil {
		value.Canonical.RecordStorageItemIDs = map[string]string{}
	}
