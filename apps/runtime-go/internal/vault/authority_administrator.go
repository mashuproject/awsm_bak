...
// changeAdministrator authors the canonical type-3 Administrator Grant or
// type-4 Administrator End Event. The target's role is derived solely from
// the authenticated Authority Parents; no local role flag is persisted.
func (r *Runtime) changeAdministrator(ctx context.Context, id, targetIDText string, grant bool) (any, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if value.Lifecycle != "Open" {
		return nil, commandError("VAULT_READ_ONLY", "A closed Vault cannot change Administrator roles.")
	}
	...
	if authority.featureSetConflict {
		return nil, commandError("AUTHORITY_CONFLICT", "Resolve the Required Feature conflict before changing Administrator roles.")
	}
	...
	if _, administrator := authority.administrators[signerMember]; !administrator || len(authority.administratorConflicts[signerMember]) > 0 {
		return nil, commandError("ADMINISTRATOR_REQUIRED", "Only an unambiguous Administrator can change Administrator roles.")
	}
	if _, active := authority.activeMembers[targetMemberID]; !active {
		return nil, commandError("MEMBER_INVALID", "The target Member is not active.")
	}
	if len(authority.administratorConflicts[targetMemberID]) > 0 {
		return nil, commandError("AUTHORITY_CONFLICT", "Resolve the target Administrator conflict before changing its role.")
	}
	_, alreadyAdministrator := authority.administrators[targetMemberID]
	if grant && alreadyAdministrator {
		return nil, commandError("ADMINISTRATOR_INVALID", "The target Member is already an Administrator.")
	}
	if !grant && !alreadyAdministrator {
		return nil, commandError("ADMINISTRATOR_INVALID", "The target Member is not an Administrator.")
	}
	...
	event, err := canonical.SignEvent(canonical.EventInput{
		...
		Type: eventType, SignerCredentialID: credentialID, AssertedAt: time.Now().UnixMilli(),
		Body: canonical.Map{0: targetMemberID[:], 1: canonicalSetValues(nil)},
	}, ed25519.PrivateKey(clientSecret.signingSecretKey))
	if err != nil {
		return nil, commandError("VAULT_EVENT_INVALID", "The Administrator role Event could not be authored.")
	}
	...
	if err != nil {
		return nil, commandError("VAULT_EVENT_INVALID", "The Administrator role Event could not be protected.")
	}
	envelope, err := storage.DecodeOpaqueEnvelope(encoded)
	if err != nil {
		return nil, commandError("VAULT_EVENT_INVALID", "The Administrator role envelope is invalid.")
	}
	if err := storeOpaqueCreationItem(r.deps.Artifacts, envelope.StorageItemID, encoded); err != nil {
		return nil, commandError("VAULT_CREATION_STORAGE_FAILED", "The Administrator role Event could not be stored.")
	}
	...
	if err := nextReplica.AdmitEvent(event, ed25519.PublicKey(clientSecret.signingPublicKey)); err != nil {
		cleanup()
		return nil, commandError("VAULT_EVENT_INVALID", "The Administrator role Event could not be admitted.")
	}
	...
	if err != nil {
		...
		return nil, commandError("VAULT_EVENT_INVALID", "The Administrator role Authority State could not be derived.")
	}
	...
	r.signal()
	return map[string]string{"targetMemberId": hexIdentifier(targetMemberID), "eventRecordId": hexIdentifier(event.RecordID), "action": action}, nil
}
